...
- [x] 우측 패널의 화면 상태를 `View Mode`(일정 목록 확인)와 `Form Mode`(새 보험 등록 및 수정)로 전환하는 로직 구현
- [x] 선택된 날짜에 이벤트가 없을 경우 나타나는 Empty State에서 '등록' 버튼 클릭 시 Form Mode로 진입하도록 연결
- [x] 캘린더 우측 상단에 보험 추가 버튼 제작, 클릭시 우측 상세 패널 Form Mode로 전환
