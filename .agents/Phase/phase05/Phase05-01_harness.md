...
### 3. 세부 통계 리스트 및 상태 분기
- [x] 시각화 자료 하단에 카테고리별 정확한 금액과 비율을 보여주는 리스트 UI 구현
- [x] 가입한 보험이 없는 경우(Empty State) 차트 대신 등록을 유도하는 안내 화면 노출
- [x] 🧪 **[Test]** 데이터가 비어있을 때 Empty State 컴포넌트가 올바르게 렌더링되는지 단위 테스트 작성

### 4. 레이아웃 및 엣지 케이스 검증
- [x] 패널 내부 스크롤 바운더리 체크 및 반응형 레이아웃 시 차트 영역이 깨지지 않는지 확인
- [x] 다른 탭으로 전환 후 복귀 시 화면 깜빡임 현상 등 UI/UX 점검
